                    ...
                    <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div>
                            <h3 class="text-lg font-medium text-gray-900">
                                Attendance Summary Report: {{ $startDate->format('M d, Y') }} to {{ $endDate->format('M d, Y') }}
                            </h3>
                            <p class="text-sm text-gray-600">Total Working Days: {{ $workingDays }}</p>
                        </div>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            @php
                                                $attendanceRate = $workingDays > 0
                                                    ? round((($user->present_count + $user->late_count) / $workingDays) * 100, 1)
                                                    : 0;
                                            @endphp
                                            <div class="flex items-center">
                                                <span class="mr-2">{{ $attendanceRate }}%</span>
...
```blade file="resources/views/admin/leave/index.blade.php"
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-6 bg-gray-50 p-4 rounded-lg">
                        <form action="{{ route('admin.leave.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($leaveRequests as $leaveRequest)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <a href="{{ route('admin.leave.show', $leaveRequest) }}" class="text-blue-600 hover:text-blue-800">
                                                View
                                            </a>
                                        </td>
                                    </tr>
